...
export const Profile = (props) => {
    ...
    const handleEmail = () => {
        setVisiEmail(!visiEmail)
        setInputEmail('')
    }

    const tambahEmail = async (e) => {
        e.preventDefault()
        try {
            const response = await axios.put(`/usersEmail/${userId}`, {
                email: inputEmail
            })
            if (response.status == 200) {
                Toast.fire({
                    icon: 'success',
                    title: 'Berhasil Edit email'
                })
                setResponse(!respon)
                setVisiEmail(!visiEmail)
                setInputEmail('')
    ...
    const tambahPass = async (e) => {
        e.preventDefault()
        try {
            const response = await axios.put(`/usersPass/${userId}`, {
    ...
    const EditProfil = async (e) => {
        ...
        try {
            if (statusUp == 2 || statusUp == 0) {
                const response = await axios.put(`/usersProfile/${userId}`, {
                    picture
                })
                if (response.status == 200) {
                    Toast.fire({
                                                                    ...
                                                                    <li className="list-group-item p-2 mb-3">
                                                                        {
                                                                            visiEmail
                                                                                ?
                                                                                <button type='button' className='btn btn-sm btn-primary' onClick={ () => handleEmail() }>Batal</button>
                                                                                :
                                                                                <button type='button' className='btn btn-sm btn-primary' onClick={ () => handleEmail() }>Edit</button>
                                                                        }
                                                                        <div className="float-right">
                                                                            {
                                                                                visiEmail
                                                                                    ?
                                                                                    <input type="email" onChange={ (e) => setInputEmail(e.target.value) } value={ inputEmail } className="form-control select2" />
                                                                                    :
                                                                                    <p>{ email }</p>
                                                                            }
                                                                        </div>
